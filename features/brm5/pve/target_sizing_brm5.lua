--[[
    Target Sizing Module for BRM5 PVE
...
-- Applies target sizing to a specific NPC model
...
-- Updates target bounds for all NPCs
function TargetSizing:updateAllTargets(npcModels)
    ...
    for _, model in ipairs(npcModels) do
        if model and model.Parent then
            ...
            if root then
                self:applyTargetSizing(model, root)
            end
        end
    end
end
